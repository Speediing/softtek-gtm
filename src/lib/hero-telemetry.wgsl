...
fn brandMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.08);
  var mark = 1e6;
  let dots = array<vec2f, 9>(
    vec2f(0.00, -0.16),
    vec2f(0.00, 0.16),
    vec2f(0.055, 0.11),
    vec2f(0.055, -0.00),
  ...
  for (var i = 0; i < 9; i = i + 1) {
    mark = min(mark, length(q - dots[i]) - 0.028);
  }
  return mark;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let lime = vec3f(0.505882, 0.737255, 0.0);
  let paper = vec3f(0.964706, 0.952941, 0.917647);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = fill * 0.08 + line * 0.28 + glow * 0.08 + traces * 0.32 + spark * 0.06;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(lime, paper, spark * 0.45 + line * 0.05);
